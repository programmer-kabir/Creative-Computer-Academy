...
const MarketplaceSubmissions = ({ taskId, userId, employeeId, addedBy, addedByRole, canManage = false }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE}api/tasks/marketplace_submissions.php`, {
        action: 'delete',
        id: id,
        deleted_by: addedBy || userId || 1
      });
      if (res.data?.status === 'success') {
        setSubmissions(prev => prev.filter(s => s.id !== id));
        setDeleteConfirmEntry(null);
      } else {
        alert(res.data?.message || 'Failed to delete marketplace submission.');
      }
      await fetchSubmissions();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 pb-1">
        ...
        {canManage && !showAddForm && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-indigo-700 hover:from-blue-500 hover:to-indigo-600 !text-[#ffffff] text-xs lg:text-sm font-bold shadow-md shadow-indigo-500/20 hover:shadow-indigo-500/30 transition-all duration-200 active:scale-95 cursor-pointer shrink-0"
          >
      ...
      {/* ── Luxury Add Form ── */}
      {canManage && showAddForm && (
        <div className="relative overflow-visible p-5 sm:p-6 rounded-3xl bg-slate-50 dark:bg-[#11192e] border border-slate-200 dark:border-indigo-500/30 shadow-md space-y-4 animate-in fade-in slide-in-from-top-3 duration-200">
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 !text-[#ffffff] text-xs lg:text-sm font-black transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2 shadow-md shadow-indigo-500/20"
            >
      ...
      {submissions.length === 0 ? (
        <div className="py-12 px-6 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-900/40 text-center space-y-3 transition-all">
          ...
          {canManage && (
            <button
              onClick={() => setShowAddForm(true)}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-600 !text-[#ffffff] text-xs font-bold hover:bg-indigo-700 shadow-sm transition-all active:scale-95 cursor-pointer mt-1"
            >
        ...
        <div className="space-y-3.5">
          {submissions.map(entry => {
            ...
            return (
              <div
                key={entry.id}
                className="group relative overflow-visible p-4 sm:p-5 rounded-2xl bg-[#ffffff] dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 hover:border-indigo-400 dark:hover:border-indigo-500/40 shadow-xs hover:shadow-md transition-all duration-300 space-y-3"
              >
                {/* ── Card Header ── */}
      ...
      {deleteConfirmEntry && createPortal(
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 animate-in fade-in duration-200">
          {/* Backdrop */}
          <div
            className="fixed inset-0 bg-black/65 backdrop-blur-sm transition-opacity"
            onClick={() => !deletingId && setDeleteConfirmEntry(null)}
          />

          {/* Modal Box */}
          <div className="relative z-10 w-full max-w-md bg-[#ffffff] dark:bg-[#0f172a] rounded-3xl border border-slate-200 dark:border-red-500/30 shadow-2xl p-6 sm:p-7 space-y-5 animate-in zoom-in-95 duration-200">
            {/* Red Accent Bar */}
            <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-red-500 via-rose-500 to-pink-500 rounded-t-3xl" />

            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-2xl bg-red-50 dark:bg-red-500/15 border border-red-200 dark:border-red-500/30 flex items-center justify-center text-red-500 shrink-0 shadow-inner">
                <FiTrash2 size={22} className="animate-pulse" />
              </div>
              <div className="space-y-1">
                <h3 className="text-base lg:text-lg font-black text-slate-900 dark:text-slate-100">
                  Delete Submission Record?
                </h3>
                <p className="text-xs lg:text-sm text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
                  Are you sure you want to remove the <strong className="text-red-600 dark:text-red-400 font-bold">{getMarketDisplay(deleteConfirmEntry)}</strong> submission? This will delete all associated activity logs permanently.
                </p>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-slate-800 dark:text-slate-200">
                  {getMarketDisplay(deleteConfirmEntry)}
                </span>
              </div>
              <span className="capitalize font-semibold text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-lg bg-slate-200/60 dark:bg-slate-700/60 text-[11px]">
                {deleteConfirmEntry.status}
              </span>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-1">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs lg:text-sm font-bold transition-all cursor-pointer disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={Boolean(deletingId)}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 !text-[#ffffff] text-xs lg:text-sm font-black shadow-lg shadow-red-500/25 transition-all duration-200 active:scale-95 disabled:opacity-50 cursor-pointer flex items-center gap-2"
              >
                {deletingId ? <FiRefreshCw size={14} className="animate-spin" /> : <FiTrash2 size={15} />}
                {deletingId ? 'Deleting...' : 'Yes, Delete Record'}
              </button>
            </div>
          </div>
        </div>,
