...
const Sidebar = ({ isOpen = true }) => {
  ...
  const menuItems = [
    ...
    { name: 'Brand Kit', transKey: 'brand_kit', path: '/brand-kit', icon: <HiSparkles size={20} className="text-amber-400" /> },
  ...
  return (
    <aside
      className={`bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 h-screen flex flex-col shadow-xl border-r border-slate-200 dark:border-slate-800 transition-all duration-300 ease-in-out z-50 overflow-hidden shrink-0 ${
        ...
        <nav className="space-y-1 px-3">
          {menuItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `group relative flex items-center justify-between px-3 py-3 rounded-xl transition-all duration-300 ease-out overflow-hidden ${isActive
